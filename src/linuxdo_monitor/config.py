...
class AppConfig(BaseModel):
    ...
    sql_admin_password: Optional[str] = Field(
        default=None,
        description="Password for SQL admin mode (allows INSERT/UPDATE/DELETE). Auto-generated if not set."
    )

    # Flask secret key for session
    flask_secret_key: Optional[str] = Field(
        default=None,
        description="Flask secret key for session management. Auto-generated if not set."
    )

    # Web management password
    web_password: Optional[str] = Field(
        default=None,
        description="Password for web management interface. Auto-generated if not set."
    )
    ...
    def convert_legacy_config(self) -> 'AppConfig':
        ...
        if not self.forums and self.bot_token:
            legacy_forum = ForumConfig(
                ...
                fetch_interval=self.fetch_interval or 30,
                enabled=True,
            )
